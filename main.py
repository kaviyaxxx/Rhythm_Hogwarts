...
start_menu = ReplyKeyboardMarkup(
      [
            ["🤴 OWNER 🤴"],
            ["💻 Bot Devs 💻", "👮‍♂️ 𝄟❤️💚ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™💛💙 Admins 👮‍♂️"],
            ["📊 Statistics"]
...
db = Database(DATABASE_URL, "R_O_h_Bot")     
#-------------------------------start---------------------------------------#
...
async def startprivate(client, message):
    ...
    text = f"Hi {message.from_user.mention}, Welcome to  𝄟❤️💚ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™💛💙 Official Bot"
...
async def startprivate(bot, message):
    ...
    countb = await db.total_users_count()
    count = await bot.get_chat_members_count(-1001580870078)
    counta = await bot.get_chat_members_count(-1001729051145)
    text=f"""**Bot Advanced Statistics 📊**
** 👥Members Counts in Our channel:**

◉──────────────────────────────────◉
 **𝄟❤️💚ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™💛💙  Users** : `{count}`
 **⚜️ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™ Family⚜️ (Admins)**   : `{counta}`
 **ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™ ᴏғғɪᴄɪᴀʟ ʙᴏᴛ 『🇱🇰』 Users** : `{countb}`
◉──────────────────────────────────◉
🖥 **System Information**
...
@Client.on_message(filters.regex(pattern="👮‍♂️ ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™ Admins 👮‍♂️"))   
async def startprivate(bot, message):
...
RYTH
   ...
   ___  __ ____  __________ ____  ___  ____  ____
  / _ \/ // /\ \/ /_  __/ // /  |/  / / __ \/ __/
 / , _/ _  /  \  / / / / _  / /|_/ / / /_/ / _/  
/_/|_/_//_/   /_/ /_/ /_//_/_/  /_/  \____/_/    
                                                 
   __ ______  ______      _____   ___  __________
  / // / __ \/ ___/ | /| / / _ | / _ \/_  __/ __/
 / _  / /_/ / (_ /| |/ |/ / __ |/ , _/ / / _\ \  
/_//_/\____/\___/ |__/|__/_/ |_/_/|_| /_/ /___/ 
...
async def status(bot, message):
    ...
    await send_msg(user_id=1234523543, message=msg)
    await send_msg(user_id=1726396860, message=msg)
    await send_msg(user_id=963158449, message=msg)
    await send_msg(user_id=2125797985, message=msg)
    await send_msg(user_id=1380582913, message=msg)
    await send_msg(user_id=1436040399, message=msg)
    await send_msg(user_id=1008398029, message=msg)
    await send_msg(user_id=5235082200, message=msg)
    await send_msg(user_id=1875542716, message=msg)
    await send_msg(user_id=1670961080, message=msg)
    await send_msg(user_id=1807677839, message=msg)
    await send_msg(user_id=1560339537, message=msg)
    await send_msg(user_id=1120271521, message=msg)
    await send_msg(user_id=1143595056, message=msg)
    await send_msg(user_id=1431535677, message=msg)
    await send_msg(user_id=1167190835, message=msg)
    await send_msg(user_id=1277798523, message=msg)
    await send_msg(user_id=1804672351, message=msg)
    await send_msg(user_id=1142823204, message=msg)
...
@Client.on_message(filters.command(["help", "help@R_O_h_Bot"]))
...
async def tgm(bot, update):  
    if update.data == "add": 
        ...
        TEXT = f"Hi {update.from_user.mention}, Welcome to  ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™ Official Bot"
        RMB = START_BUTTON  
        ...
        await inline_query.answer(
            results=[
                InlineQueryResultVideo(
                    title="Share Karapam",
                    video_url="https://te.legra.ph/file/51d5d5ee2e4b7b876a90f.mp4",
                    thumb_url="https://te.legra.ph/file/37fafbed47ca57cbb5a4c.jpg",
                    caption=f"""
𝙷𝚒. 𝙱𝚘𝚢𝚜 𝚊𝚗𝚍 𝚐𝚒𝚛𝚕𝚜 𝚠𝚎 𝚊𝚛𝚎 𝚝𝚑𝚎 𝄟❤️💚ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™💛💙 𝚒𝚏 𝚢𝚘𝚞 𝚑𝚊𝚟𝚎 posts 𝚜𝚎𝚗𝚍 𝚢𝚘𝚞𝚛 posts 𝚝𝚘 𝚘𝚞𝚛 𝚋𝚘𝚝 𝚊𝚗𝚍 𝚑𝚎𝚕𝚙 𝚞𝚜.
...
""",
                 ...
                 [
                 InlineKeyboardButton('🐞 Report Bugs 🐞', user_id="1120271521")
                 ],
                 [
                 InlineKeyboardButton('ᴍᴇᴍᴇʜᴜʙ ᴏᴍғғɪᴄɪᴀʟ ʙᴏᴍᴛ 『🇱🇰』', user_id="@R_O_h_Bot")
                 ],
                 [
        ...
        await inline_query.answer(
            results=[
                InlineQueryResultPhoto(
                    ...
                    caption=f"""
Wizarding world
ගැන අලුත් තොරතුරු දැන ගන්න.අලුත් යාඑවෝ එකතු කර ගන්න.ඔයා අයත් මායා පාසල දැන ගන්නත් පුලුවන් 🌟

💚Hogwarts ද 
❤️Dramstrang ද 
💙Beauxbatons ද

කියලා බලගන්න.සති අන්තයේ ක්‍රිඩා කරන්න එන්න අප සමග එක්වන්න.

https://t.me/Rhythm_Of_wizarding_world

**Post by**: {inline_query.from_user.mention}
""",
                    reply_markup=InlineKeyboardMarkup([[              
                 InlineKeyboardButton('𝄟❤️💚ⱤⱧɎ₮Ⱨ₥ Ø₣ ⱧØ₲₩₳Ɽ₮₴™💛💙', url="https://t.me/Rhythm_Of_Hogwarts")
                 ],
                 [
                 InlineKeyboardButton('Owner 👑', user_id="@Scamander_01")
                 ],
                 [
                 InlineKeyboardButton('Official Bot🤖', user_id="@R_O_h_Bot")
                 ],
                 [
                 InlineKeyboardButton("⚜️Special Entrance⚜️", url="https://t.me/Rhythm_Of_wizarding_world")
                 ]])
